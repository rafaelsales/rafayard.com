...
- Colors in `src/css/style.css`: light is lifted from pages.rafayard.com/secretaria-lar (warm gray `#e6e6e3` page, white cards, `#17181a` ink, green `#2a6b3e` links); dark from world.hey.com's dark mode (`#1b2733`/`#ece9e6`/`#50a2ff`). Themes switch via a `data-theme` attribute stamped by an inline `<head>` script before first paint and toggled by the top-right button (persisted in `localStorage`); a `prefers-color-scheme` media query is the no-JS fallback. Those two inline snippets in `base.njk` are the only allowed client-side JavaScript — add no more, and no webfonts: perfect Core Web Vitals are part of the SEO strategy.
